...
class StateRequirement(Base):
    ...
    expedited_fee: Mapped[Optional[int]] = mapped_column(Integer)
    name_reservation_fee: Mapped[Optional[int]] = mapped_column(Integer)

    # Processing
    standard_processing_days: Mapped[int] = mapped_column(Integer, nullable=False)
    expedited_processing_days: Mapped[Optional[int]] = mapped_column(Integer)
    ...
    min_directors: Mapped[Optional[int]] = mapped_column(Integer)  # For corps

    # Annual compliance
    annual_report_fee: Mapped[Optional[int]] = mapped_column(Integer)
    annual_report_month: Mapped[Optional[int]] = mapped_column(Integer)  # 1-12
    franchise_tax: Mapped[Optional[int]] = mapped_column(Integer)

    # Filing details
    filing_agency: Mapped[str] = mapped_column(String(200), nullable=False)
    filing_agency_url: Mapped[Optional[str]] = mapped_column(String(500))
    online_filing_available: Mapped[bool] = mapped_column(Boolean, default=False)
    naming_rules: Mapped[Optional[dict]] = mapped_column(JSON)  # Required suffixes, restricted words
    required_documents: Mapped[Optional[dict]] = mapped_column(JSON)  # List of required docs
